...
@Component({
  ...
  template: `
    ...
    <div class="flex items-center gap-4 mb-6">
      <p-button icon="pi pi-arrow-left" [rounded]="true" [text]="true" severity="secondary" routerLink="/cursos" />
      <h2 class="m-0 text-color font-bold text-2xl">{{ isEdit() ? 'Editar Curso' : 'Novo Curso' }}</h2>
    </div>
    <div class="card">
      <form [formGroup]="form" (ngSubmit)="onSubmit()">
        ...
        <div class="flex gap-3 mt-6">
          ...
          <p-button label="Cancelar" icon="pi pi-times" severity="secondary" [outlined]="true" routerLink="/cursos" />
        </div>
      </form>
    </div>
  `,
